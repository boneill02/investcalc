let PERIODS = 52; // paycheck every week
...
function getReturn(investment, yr, rate) {
    return (
        FV(rate / PERIODS, yr * PERIODS, investment, 0, 0) * -1 -
        investment * PERIODS * yr
    );
}

function getInputs() {
    if (wrapper.contains(results)) {
        results.remove();
    }
    if (!wrapper.contains(inputs)) {
...
function display() {
    ...
    document.getElementById("inc6T").innerHTML =
        "$" + numberWithCommas(inc6 * TAX_FACTOR);
    document.getElementById("inc9T").innerHTML =
        "$" + numberWithCommas(inc9 * TAX_FACTOR);
    document.getElementById("inc12T").innerHTML =
        "$" + numberWithCommas(inc12 * TAX_FACTOR);
}
